...
export function renderLogin(container) {
  ...
  forms.crew.addEventListener('submit', async (e) => {
    ...
    await handleLogin('crew', { pin, branchId: parseInt(branchId) });
  });
}
...
async function handleLogin(role, credentials) {
  showLoading();
  try {
    const payload = { role, pin: credentials.pin };
    if (role === 'crew') {
      payload.branch_id = credentials.branchId;
    }
